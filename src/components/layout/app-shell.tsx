...
export function AppShell({ workspace, workspaces, user, children }: ShellProps) {
  ...
  function sidebar(mobile = false) {
    ...
    return <aside className={`flex h-full flex-col border-r border-[var(--border)] bg-[var(--muted-surface)] px-4 py-5 ${mobile ? "w-[min(280px,85vw)]" : "w-[232px]"}`}>
      ...
      <label className="relative mb-7 block"><span className="sr-only">Aktywna przestrzeń</span><select value={workspace.id} onChange={(event) => { setOpen(false); router.push(`/w/${event.target.value}/dashboard`); }} className="tf-input h-11 w-full appearance-none pr-9 text-sm font-medium">{workspaces.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select><ChevronsUpDown className="pointer-events-none absolute top-3.5 right-3 text-[var(--muted)]" size={15} aria-hidden="true" /></label>
