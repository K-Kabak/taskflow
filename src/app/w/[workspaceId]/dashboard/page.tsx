...
export default async function DashboardPage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  const [projects, myOpen, overdue, done, recent, upcoming, byStatus, latestActivity] = await Promise.all([
    ...
    db.task.groupBy({ by: ["status"], where: { project: { workspaceId, archivedAt: null } }, _count: { _all: true } }),
    db.activity.findFirst({ where: { workspaceId, project: { workspaceId, archivedAt: null } }, orderBy: { createdAt: "desc" }, select: { createdAt: true } }),
  ]);
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6"><ProgressMetrics title="Postęp przestrzeni" counts={counts} overdue={overdue} lastActivity={latestActivity?.createdAt.toLocaleString("pl-PL", { timeZone: "Europe/Warsaw" })} /></div>
