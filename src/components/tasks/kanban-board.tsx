...
function TaskCard({ task, workspaceId, projectId, overlay, dragProps }: { task: BoardTask; workspaceId: string; projectId: string; overlay?: boolean; dragProps?: React.HTMLAttributes<HTMLButtonElement> }) {
  ...
  return <article className={`rounded-2xl border border-[#e9e9e4] bg-white p-4 shadow-[0_6px_20px_rgba(37,37,37,0.05)] ${overlay ? "rotate-2 shadow-xl" : ""}`}><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-1.5 text-xs text-[var(--muted)]"><CalendarDays size={13} aria-hidden="true" />{formatDateOnly(task.dueDate)}</span>{dragProps && <button {...dragProps} type="button" aria-label={`Przenieś zadanie ${task.title}`} className="min-h-9 min-w-9 cursor-grab rounded-lg p-2 text-[#666660] hover:bg-[var(--muted-surface)] active:cursor-grabbing"><GripVertical size={16} /></button>}</div><Link href={`/w/${workspaceId}/projects/${projectId}?task=${task.id}`} className="mt-3 block font-semibold leading-snug hover:text-[var(--accent-ink)]">{task.title}</Link><div className="mt-4 flex flex-wrap gap-1">{task.labels.slice(0, 2).map((label) => <span key={label.id} className="rounded-lg border border-[#ececea] px-2 py-1 text-[11px]" title={label.name}><span className="mr-1 inline-block size-2 rounded-full" style={{ backgroundColor: label.color }} />{label.name}</span>)}<span className={`rounded-lg px-2 py-1 text-[11px] font-medium ${priorityClass}`}>Priorytet: {priorities[task.priority]}</span></div><div className="mt-4 flex items-center border-t border-[var(--border)] pt-3"><div className="flex -space-x-2" aria-label={task.assignees.length ? `Przypisani: ${task.assignees.map((person) => person.name).join(", ")}` : "Nikt nie jest przypisany"}>{task.assignees.slice(0, 3).map((person) => <span title={person.name} key={person.id} className="grid size-7 place-items-center rounded-full border-2 border-white text-[9px] font-semibold text-white" style={{ backgroundColor: person.color }}>{person.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span>)}{task.assignees.length > 3 && <span className="grid size-7 place-items-center rounded-full border-2 border-white bg-[#eeeeca] text-[10px]">+{task.assignees.length - 3}</span>}</div>{!task.assignees.length && <span className="text-xs text-[var(--muted)]">Bez przypisania</span>}</div></article>;
}
